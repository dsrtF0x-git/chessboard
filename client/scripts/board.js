...
Board.prototype.findRook = function(king) {
  if (king.color === bottomPlayerColor) {
    if (Util._arrayEquals(king.currentPosition, whiteKingCoordinates.rightCastlePos)) {
      console.log(king.currentPosition + "current pos")
...
Board.prototype.replacePiece = function(piece) {
  if (!piece) {
    return null;
  }
  const newPiece = pieceMapping.get(piece.name);

  return new newPiece(piece.color, this, piece.currentPosition, piece.moved);
};